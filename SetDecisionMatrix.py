...
class SetDecisionMatrix:
    ...
    class TargetSDM(SDM):
        ...
        def best(self, true=True) -> object:
        ...
        def insert(self, value):
            ...
            if self.root is None:
                ...
                self.curr = self.root
                self.count += 1
            elif self.improving_only:
                self.imp_insert(value)
            elif not self.improving_only:
        ...
        def rec_add(self, value, pnode, anode):
            if anode.key == pnode.key:
                return None
            if pnode.is_better_than(anode.key, self.target):  # parent > added node –> __bad__
                if pnode.bad is None:
                    pnode.__bad__(anode)
                    self.count += 1
                    return None
                else:
                    if self.rec_add(value, pnode.bad, anode) is None:
                        return None
            else:  # added node > parent –> __good__
                if pnode.good is None:
                    pnode.__good__(anode)
                    self.count += 1
                    return None
                else:
                    if self.rec_add(value, pnode.good, anode) is None:
                        return None
    ...
    class TwoDimGridKeyFormula(DefaultKeyFormula):
        ...
        def __delta__(self, value: any, curr_pos: list[int], new_pos: list[int]):
            """
...
def main():
    sdm = SetDecisionMatrix.TargetSDM(target=16, imp_only=False)
